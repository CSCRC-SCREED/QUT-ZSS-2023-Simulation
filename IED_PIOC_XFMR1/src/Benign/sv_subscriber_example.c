/*
 * SV subscriber for receiving 7 physical values which are only applicable for IED_XFMR1
 */
...
const char* subasdu[7] = {"66kV1", "66kV2", "XFMR1W1", "XFMR1W2", "CB_XFMR1", "F_66kV1", "F_XFMR1"};
...
svUpdateListener (SVSubscriber subscriber, void* parameter, SVSubscriber_ASDU asdu)
{
    ...
    for (int i = 0; i<7; i++) {
	    ...
	    { 
	  	 ...
	  	 if (i == 0) { // write the first measurement, always fresh start
	  	 	//open File with "w"//
    			fp=fopen("/home/ray/Desktop/XFMR1 OverCur Prot/TripRecord/CurrentValues.csv","w"); 
    		...
    		else {  // append the rest measurements after
    			//open File with "a"//
    			fp=fopen("/home/ray/Desktop/XFMR1 OverCur Prot/TripRecord/CurrentValues.csv","a"); 
