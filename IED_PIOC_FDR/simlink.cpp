...
int main()
{
	...
	while(1)
	{
		...
		//printf("CB1_22kV: %d A\n CB2_22kV: %d A\n CB3_22kV: %d A\n FDR1: %d A\n FDR2: %d A\n FDR3: %d A\n FDR4: %d A\n Reset all CB: %d\n  Trip CB1_22kV: %d\n Trip CB2_22kV: %d\n Trip CB3_22kV: %d\n Trip CB_FDR1: %d\n Trip CB_FDR2: %d\n Trip CB_FDR3: %d\n Trip CB_FDR4: %d\n\n", stations_data[0].analogIn[0], stations_data[0].analogIn[1], stations_data[0].analogIn[2], stations_data[0].analogIn[3], stations_data[0].analogIn[4], stations_data[0].analogIn[5], stations_data[0].analogIn[6], stations_data[0].digitalIn[0], stations_data[0].digitalOut[0], stations_data[0].digitalOut[1], stations_data[0].digitalOut[2], stations_data[0].digitalOut[3], stations_data[0].digitalOut[4], stations_data[0].digitalOut[5], stations_data[0].digitalOut[6]);
		

		FILE *fp;
	    	int i;
	    	int CBstval [7] = {0,1,0,0,0,0,0};
	    	
	    	for (i=0; i<7; i++)
	    	    CBstval [i] = stations_data[0].digitalOut[i];

//################## write CB siginals and read by GOOSE_pub ####################//  
	    //Open File//
...
//################## read current values and send to OpenPLC runtime ####################// 
	    //Open File//
	    	fp=fopen("TripRecord/CurrentValues.csv","r");
	    	if(fp==NULL) {
		    printf("File cannot open! " );
		    exit(0);
	     	}
		...
		for (i=0; i<13; i++) {
		    fscanf(fp,"%hd,", &stations_data[0].analogIn[i]);
		    //Debug//
		    //printf("%hd\t",stations_data[0].analogIn[i]);
		}    
	    //Close file//
	    	fclose(fp);
