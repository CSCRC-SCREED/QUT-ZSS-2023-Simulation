...
gooseListener(GooseSubscriber subscriber, void* parameter)
{
    ...
    for (int j=0; buffer[j] != '\0'; j++) {
        //Debug
        //printf("%c", buffer[j]);
        if ( (buffer[j] == ',') && (buffer[j-2] == 's') && (i<5) ){
            CBstval[i] = 0;
            //Debug
            //printf("%d,%d,%d\n", i, j, CBstval[i]);
            i++; 
        }
        if ( (buffer[j] == ',') && (buffer[j-2] == 'u') && (i<5) ){
            CBstval[i] = 1;
            //Debug
            //printf("%d,%d,%d\n", i, j, CBstval[i]);
            i++;        
...
//############### store CBstval to local for control signal ##########################//  
    //Open File//
    fp=fopen("/home/ray/Desktop/Primary Plant/TripRecord/TripFromBFP.csv","w");	
    if(fp==NULL) {
...
main(int argc, char** argv)
{
    ...
    GooseSubscriber subscriber = GooseSubscriber_create("QUTZS_FDRBFP/LLN0$GO$gcb_2", NULL);

    GooseSubscriber_setAppId(subscriber, 0x3113);
